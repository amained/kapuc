...
#define expect_return(tok, t_expected, callback) expect(tok, t_expected, callback, {return;})
#define expect(tok, t_expected, callback_success, callback_failure) if (tok.t != t_expected) {callback_failure} else callback_success

// declare some functions needed because c
struct AST parse_expr(struct Parser *p);
struct AST parse_list_decl(struct Parser *p, struct AST (*f)(struct Parser *));
...
struct AST parse_atom(struct Parser *p) {
    ...
    switch (tok.t) {
        ...
        case AMPERSAND:
            ...
            struct AST ast7 = {.sub_value = 0, .nodes = NULL, .node_type = E_UNARY}; // 0: REFERENCE
            arrput(ast7.nodes, ast6);
            return ast7;
        case LPAREN:
            p->pos++;
            struct AST ast3 = parse_expr(p);
            expect(p->tokens[p->pos], RPAREN, {
                p->pos++;
                return ast3;
            }, {log_error("Expected )");})
            p->error = true;
        ...
        case LBRACKET:
            ...
            expect(p->tokens[p->pos], RBRACKET, {
                p->pos++;
                return ast4;
            }, {log_error("Expected ]");log_debug("current token pos: %d", p->pos);})
            p->error = true;
            return (struct AST) {.value = NULL, .node_type = -1};
        case DOTDOT:
            p->pos++;
            struct AST ast5 = {.value = NULL, .node_type = E_DOTS}; // variadic/other shit
            return ast5;
        default:
            return (struct AST) {.value = NULL, .node_type = -1};
    }
...
int8_t calc_precedence(enum TOK_TYPE t) {
    switch (t) {
        case PLUS:
        case MINUS:
            return 1;
...
struct AST parse_expr_1(struct AST left, struct Parser *p, int8_t precedence) {
    if (left.node_type == -1) {
        return left;
    }
    if (p->pos >= arrlen(p->tokens)) {
    ...
    while (p->pos < arrlen(p->tokens) && calc_precedence(op.t) >= precedence) {
        op = p->tokens[p->pos];
        if (calc_precedence(op.t) == -1) return left;
        p->pos++;
        ...
        struct AST new_tree = {.sub_value = op.t - 21, .nodes = NULL, .node_type = E_UNARY}; // 0: PLUS, 1: MINUS, 2: MULTIPLY, 3: DIVIDE
...
struct AST parse_type_decl(struct Parser *p, struct AST (*f)(struct Parser *)) {
    struct AST tree = f(p);
    if (tree.node_type == -1) return (struct AST) {.node_type = -1};
    if (tree.sub_value == 0 && tree.node_type == E_UNARY) return tree; // REFERENCE
    if (tree.node_type == E_DOTS) return tree; // variadic
    expect(p->tokens[p->pos], COLON, {p->pos++;}, {return (struct AST) {.node_type = -1};});
    struct AST type = parse_atom(p);
    arrput(tree.nodes, type);
    return tree;
}

struct AST parse_let_decl(struct Parser *p){
    struct AST tree = {.node_type = S_LET};
    p->pos++;
    expect(p->tokens[p->pos], IDENT, {tree.value = p->tokens[p->pos].s; p->pos++;}, {return (struct AST) {.node_type = -1};});
    return tree;
}

struct AST parse_coffee(struct Parser *p){
    struct AST tree = {.node_type = S_COFFEE};
    p->pos++;
    tree.value = p->tokens[p->pos-1].s;
    expect(p->tokens[p->pos], LPAREN, {p->pos++;}, {return (struct AST) {.node_type = -1};});
    struct AST args = parse_list_decl(p, &parse_expr);
    arrput(tree.nodes, args);
    expect(p->tokens[p->pos], RPAREN, {p->pos++;}, {return (struct AST) {.node_type = -1};});
    return tree;
}

struct AST parse_block(struct Parser *pParser);
...
struct AST parse_statement(struct Parser *p){
    ...
    switch(token.t) {
        case LET: {
            ...
            expect(p->tokens[p->pos], SEMICOLON, {p->pos++;}, {
                // with expression
                expect(p->tokens[p->pos], EQ, {p->pos++;}, {return (struct AST) {.node_type = -1};});
                struct AST expression = parse_expr(p);
                arrput(tree.nodes, expression);
                expect(p->tokens[p->pos], SEMICOLON, {p->pos++;}, {return (struct AST) {.node_type = -1};});
            });
            return tree;
        }
        case COFFEE: {
...
struct AST parse_types(struct Parser *p){
    struct AST tree = {.node_type = E_LIST};
...
struct AST parse_func_decl(struct Parser *p){
    ...
    expect(p->tokens[p->pos], RPAREN, {p->pos++;}, {return (struct AST) {.node_type = -1};});
    expect(p->tokens[p->pos], COLON, {p->pos++;}, {return (struct AST) {.node_type = -1};});
    expect(p->tokens[p->pos], IDENT, {arrput(tree.nodes, parse_atom(p));}, {return (struct AST) {.node_type = -1};});
    return tree;
}
...
struct AST parse_block(struct Parser *p) {
    if (p->tokens[p->pos].t == LBRACE) {
        ...
        p->pos++;
        if (p->tokens[p->pos].t == RBRACE) return tree;
        while (p->pos < arrlen(p->tokens)) {
