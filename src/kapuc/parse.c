...
struct AST parse_atom(struct Parser *p) {
  ...
  case AMPERSAND:
    ...
    struct AST ast7 = {
        .sub_value = 0, .nodes = NULL, .node_type = E_UNARY}; // 0: REFERENCE
...
struct AST parse_expr_1(struct AST left, struct Parser *p, int8_t precedence) {
  ...
  while (p->pos < arrlen(p->tokens) && calc_precedence(op.t) >= precedence) {
    ...
    struct AST new_tree = {
        .sub_value = op.t - 21,
        .nodes = NULL,
        .node_type = E_UNARY}; // 0: PLUS, 1: MINUS, 2: MULTIPLY, 3: DIVIDE
...
struct AST parse_statement(struct Parser *p) {
  ...
  case LET: {
    ...
    expect(
        p->tokens[p->pos], SEMICOLON, { p->pos++; },
        {
          // with expression
          expect(
              p->tokens[p->pos], EQ, { p->pos++; },
              { return (struct AST){.node_type = -1}; });
...
struct AST parse_func_decl(struct Parser *p) {
  ...
  expect(
      ...
      { return (struct AST){.node_type = -1}; });
  expect(
      p->tokens[p->pos], IDENT, { arrput(tree.nodes, parse_atom(p)); },
      { return (struct AST){.node_type = -1}; });
  return tree;
}
