...
// this need to be done before doing codegen because our ast suck, and we need
// to make it more descriptive BIG NOTE: *tree in this context is actually a
// pointer to the tree and not array/stb_ds

struct CodeTreeNode generate_int(struct AST *tree) {
  struct CodeTreeNode node = {
      .type = d_int, .children = NULL, .value = tree->value};
  return node;
}
...
struct CodeTreeNode generate_return(struct AST *tree) {
  struct CodeTreeNode node = {
      .type = t_return, .children = NULL, .value = NULL};
...
struct CodeTreeNode generate_type(struct AST *tree) {
  struct CodeTreeNode node = {
      .type = d_type, .children = NULL, .value = tree->value};
  return node;
}

struct CodeTreeNode generate_function_types(struct AST *tree) {
  struct CodeTreeNode node = {
      .type = t_types, .children = NULL, .value = tree->value};
  return node;
}
...
struct CodeTreeNode generate_block(struct AST *tree) {
  ...
  if (tree->node_type == B_STMTS) {
    struct CodeTreeNode node = {
        .type = t_block, .children = NULL, .value = NULL};
...
struct CodeTreeNode generate_function(struct AST *tree) {
  struct CodeTreeNode node = {
      .type = t_func, .children = NULL, .value = tree->value};
